...
export function addTransaction(transaction, userID) {
	return function(dispatch) {
		...
		return axiosWithAuth()
			.post(
				`https://lambda-budget-blocks.herokuapp.com/api/manual/transaction/${userID}`,
				transaction
			)
			.then(response => {
...
// delete trans
export function deleteTrans(transaction, userID, transID) {
	return function(dispatch) {
		...
		return axiosWithAuth()
			...
			.then(response => {
				dispatch({ type: DELETE_TRANS_SUCCESS, payload: response.data });
			})
			.catch(error => {
				dispatch({

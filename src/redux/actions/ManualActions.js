import { axiosWithAuth } from '../../components/old_components/AxiosWithAuth';
import environmentUrls from '../../dispatch';

export const ADD_DEFAULT_CATEGORIES_LOADING =
  'ADD_DEFAULT_CATEGORIES_LOADING';
export const ADD_DEFAULT_CATEGORIES_SUCCESS =
  'ADD_DEFAULT_CATEGORIES_SUCCESS';
export const ADD_DEFAULT_CATEGORIES_FAILED =
...
export const addManualBlockSuccess = (obj) => ({
  type: ADD_MANUAL_BLOCKS_SUCCESS,
  payload: obj,
});
...
export const editManualBlockSuccess = (obj) => ({
  type: EDIT_MANUAL_BLOCKS_SUCCESS,
  payload: obj,
});
  ...
  return function (dispatch) {
    ...
    return (
      axiosWithAuth()
        ...
        .get(`${environmentUrls.base_url}/manual/onboard/${userID}`)
        .then((response) => {
          // After back end creates the default categories: get all the categories from back end
          return axiosWithAuth()
            .get(
              `${environmentUrls.base_url}/api/users/categories/${userID}`
            )
            .then((response) => {
...
export function addManualBlocks(userId, obj) {
  ...
  return function (dispatch) {
    axiosWithAuth()
      .post(`${environmentUrls.base_url}/manual/categories/${userId}`, obj)
      .then((i) => {
...
export function getManualTrans(userID, history) {
  ...
  return function (dispatch) {
    ...
    return axiosWithAuth()
      .get(`${environmentUrls.base_url}/manual/transaction/${userID}`)
      .then((response) => {
        ...
        const data = {
          accounts: [],
          Categories: response.data.list.filter(
            (cat) => cat.budget !== null
          ),
        };
